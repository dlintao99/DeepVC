...
def prepare_vocab(sentences):
    ...
    for i, row in enumerate(sentences):
        ...
        # import nltk
        # nltk.download('punkt')
        # 如果nltk.download('punkt')不成功,解决方法如下
        # https://github.com/nltk/nltk/issues/1787  还是没解决...
...
def prepare_gt(anno_data):
    ...
    val_txt = sorted(val_txt,key = lambda x: int(x.split('\t')[0]))
    test_txt = sorted(test_txt, key = lambda x: int(x.split('\t')[0]))
    for text in val_txt:
        val_reference_txt.write(text)
    for text in test_txt:
        test_reference_txt.write(text)
