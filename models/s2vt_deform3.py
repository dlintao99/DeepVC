...
class S2VT_deform3(nn.Module):
    ...
    def __init__(self, 
        ...
        super(S2VT_deform3, self).__init__()
        ...
        self.LSTM_unit_decoder = nn.LSTMCell(self.dim_hidden_state + self.dim_embedded_word, self.dim_hidden_state)
    ...
    def forward(self, feats_videos, labels, teacher_forcing_ratio = 1.0):
        ...
        hidden_state_h_decoder = torch.zeros(batch_size, self.dim_hidden_state, device = self.DEVICE)
        hidden_state_c_decoder = torch.zeros(batch_size, self.dim_hidden_state, device = self.DEVICE)
        ...
        if flag_apply is False:
            ...
            for i in range(self.num_time_steps_decoder):
                input_LSTM_unit_decoder = torch.cat([outputs_encoder_h, embedding_input_words_decoder], dim = 1)
